...
c     to create obstacles in z direction(maximum number100)  
      
      integer fpds10
      integer center_number
      real fpds9,fpds11(100)
      real width,obstacle_center(100)
...
c
         ...
         tscale=fpds6
         width=fpds9
         center_number=fpds10
         obstacle_center=fpds11
...
c
                  if (iplane.eq.0) then
                     chi1 = chi(h,radius,width,smooth,xc11,zc,yc,
     &                    x1center,x2center,
     &                    center_number,obstacle_center)
                     chi2 = chi(h,radius,width,smooth,xc22,zc,yc,
     &                    x1center,x2center,
     &                    center_number,obstacle_center)
c     
...
c     
                  elseif (iplane.eq.1) then
                     chi1 = chi(h,radius,width,smooth,xc11,yc,zc,
     &                    x1center,x2center,
     &                    center_number,obstacle_center)
                     chi2 = chi(h,radius,width,smooth,xc22,yc,zc,
     &                    x1center,x2center,
     &                    center_number,obstacle_center)
                  end if
      ...
      real function chi(xLB,yLB,width,s,x1,y1,x3,x1c,x2c,
     &      center_number,zcenter)
c
...
C     endif
      ...
      if ((x1.le.(x1c+xLB/2)).and.(x1.ge.(x1c-xLB/2))) then
      	if ((y1.le.(x2c+yLB/2)).and.(y1.ge.(x2c-yLB/2))) then
      		if((x3.le.(0.5)).and.(x3.ge.(0.5))) then
      			chi = 1.
      		else
      			chi = 0.
      		endif
